...
Command: npx gltfjsx@6.2.16 newcar.gltf 
...
export function Newcar(props) {
  const { nodes, materials } = useGLTF('/newcar/newcar.gltf')
  const newcarRef=useRef();
  useFrame(()=>{
   if(newcarRef.current){
    newcarRef.current.rotation.y+=0.01;
   }
   if(newcarRef.current.position.y>(props.minlength))
   newcarRef.current.position.y-=0.1;
  })
  return (
    <group {...props} dispose={null} ref={newcarRef} position={[0,2,0]}>
